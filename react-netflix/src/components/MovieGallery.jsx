...
const MovieGallery = ({ title, searchTerm }) => {
  ...
  const token =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2NTg1OWZhOGI5ODkwODAwMTg0ODg3YTAiLCJpYXQiOjE3MDMyNTU5NzYsImV4cCI6MTcwNDQ2NTU3Nn0.6fe0GaqNkCMs0HAibZ82DEInkLnHOGBQ07NrQ1UZ1BA"; // Inserisci il tuo token
  ...
  useEffect(() => {
    const fetchComments = async () => {
      ...
      try {
        const response = await fetch(
          `https://striveschool-api.herokuapp.com/api/comments/${selectedFilmId}`,
          {
            headers: {
  ...
  return (
    <div className="container mt-4">
      ...
      {selectedFilmId && (
        <div>
          <h3 style={{ color: "#d81f26" }}>
            Comments for{" "}
            {movies.find((movie) => movie.imdbID === selectedFilmId)?.Title}
          </h3>
